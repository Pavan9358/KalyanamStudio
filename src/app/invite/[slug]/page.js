...
function RSVPForm({ invitationId, config }) {
  ...
  const handleSubmit = (e) => {
    ...
    // Direct to WhatsApp
    const text = `🎉 RSVP for ${form.name}:\n✅ Attending: ${form.attending ? 'YES' : 'NO'}\n👥 Guests: ${form.guest_count}\n✉️ Message: ${form.message}`;
    window.open(`https://wa.me/${config.rsvp_whatsapp || '919358356885'}?text=${encodeURIComponent(text)}`, '_blank');
  ...
  if (status === 'success') {
    return (
      <div className={styles.rsvpSuccess}>
        ...
        <h3 className={styles.sectionTitle}>RSVP Sent!</h3>
        <p>Thank you for letting us know. See you there!</p>
      </div>
    );
  }
  ...
  return (
    <div className={styles.rsvpPostcard}>
      ...
      <form onSubmit={handleSubmit} className={styles.rsvpForm}>
        ...
        />
        <button type="submit" className={styles.rsvpWaxBtn}>
          Send Response
        </button>
      </form>
    </div>
...
export default function PublicInvitePage({ params }) {
  ...
  const [theme, setTheme] = useState('dark'); // Default to Midnight Dark
  const [playing, setPlaying] = useState(false);
  const [selectedImage, setSelectedImage] = useState(null); // Added for Gallery Lightbox
    ...
    const load = async () => {
      ...
      if (slug.startsWith('demo-')) {
        ...
        setData({
          ...
          events: [
            { name: 'Sacred Wedding', date: '2026-02-22', time: '10:30', venue: 'Royal Palace Grounds', venue_link: 'https://maps.google.com' },
            { name: 'Grand Reception', date: '2026-02-22', time: '19:00', venue: 'Grand Ballroom', venue_link: 'https://maps.google.com' }
          ],
          ...t.template_json
        });
        setLoading(false);
    ...
    <div className={`${styles.page} ${styles[theme]}`}>
      ...
      {opened && (
        <main>
          ...
          {/* GALLERY - Arched Images & Lightbox */}
          ...
          <section className={styles.invSection} style={{ borderTop: '1px solid rgba(255,230,183,0.1)' }}>
            <motion.div initial={{opacity: 0, y: 40}} whileInView={{opacity: 1, y: 0}} viewport={{once: true}}>
              ...
              <RSVPForm invitationId={slug} config={template.template_json} />
            </motion.div>
          </section>
